...
def load_vocab(fname):
	...
	:return:
	"""
    vocab = [line.split()[0] for line in open(fname, 'r',encoding='utf-8').readlines() if int(line.split()[1]) > hp.min_cnt]
...
def load_train_data():
    de_sents = [regex.sub("[^\s\p{Latin}']", "", line) for line in open(hp.source_train, 'r', encoding='utf-8').readlines() if
                line[0] != "<"]
    en_sents = [regex.sub("[^\s\p{Latin}']", "", line) for line in open(hp.target_train, 'r', encoding='utf-8').readlines() if
                line[0] != "<"]
